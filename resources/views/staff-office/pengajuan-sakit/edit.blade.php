        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title">Edit Pengajuan Sakit</h5>
                </div>
                <div class="card-body">
                    <form id="sakitForm" action="{{ route('staff-office.pengajuan-sakit.update', $sakit->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="mulai_sakit">Mulai Sakit</label>
                            <input type="date" class="form-control" id="mulai_sakit" name="mulai_sakit" value="{{ $sakit->mulai_sakit }}" required>
                        </div>
                        <div class="form-group">
                            <label for="selesai_sakit">Selesai Sakit</label>
                            <input type="date" class="form-control" id="selesai_sakit" name="selesai_sakit" value="{{ $sakit->selesai_sakit }}" required>
                        </div>
                        <div class="form-group">
                            <label for="alasan">Alasan</label>
                            <textarea class="form-control" id="alasan" name="alasan" required>{{ $sakit->alasan }}</textarea>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" id="disetujui_oleh" name="disetujui_oleh" required>
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{ $sakit->disetujui_oleh == $user->id ? 'selected' : '' }}>{{ $user->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" id="diketahui_oleh" name="diketahui_oleh" required>
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{ $sakit->diketahui_oleh == $user->id ? 'selected' : '' }}>{{ $user->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <button type="submit" class="btn btn-primary">Update Pengajuan Sakit</button>
                    </form>
                </div>
            </div>
        </div>
