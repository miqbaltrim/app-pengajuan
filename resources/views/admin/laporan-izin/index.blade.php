        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="card-title">Laporan Izin Karyawan</h5>
                    <div class="ml-auto">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        <form action="{{ route('admin.laporan-izin.search') }}" method="POST" class="form-inline">
                            @csrf
                            ...
                            <a href="{{ route('admin.laporan-izin.riwayat') }}" class="btn btn-custom-orange mb-2">Riwayat Izin</a>
                            <a href="{{ route('admin.approve.izin') }}" class="btn btn-success mb-2">Approve Izin</a>
                        </form>
                    </div>
                    ...
                    <div class="table-responsive" id="printableTable">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                @foreach($laporanIzin as $laporan)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $laporan['nama'] ?? '' }}</td>
                                    <td>{{ $laporan['tahun'] ?? '' }}</td>
                                    <td>{{ $laporan['position'] ?? '' }}</td>
                                    <td>{{ $laporan['jumlah_hari_izin'] ?? '' }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
