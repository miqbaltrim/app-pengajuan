                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($kasbon->setujui === 'tunggu' && $kasbon->disetujui_oleh == auth()->user()->id)
                                            <form action="{{ route('admin.kasbon.disetujuiTerima', $kasbon->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit" class="btn btn-success btn-sm">Setujui</button>
                                            </form>
                                            <form action="{{ route('admin.kasbon.disetujuiTolak', $kasbon->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit" class="btn btn-danger btn-sm">Tolak</button>
                                            </form>
                                        @elseif($kasbon->ketahui === 'tunggu' && $kasbon->diketahui_oleh == auth()->user()->id)
                                            <form action="{{ route('admin.kasbon.diketahuiTerima', $kasbon->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit" class="btn btn-success btn-sm">Setujui</button>
                                            </form>
                                            <form action="{{ route('admin.kasbon.diketahuiTolak', $kasbon->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit" class="btn btn-danger btn-sm">Tolak</button>
                                            </form>
