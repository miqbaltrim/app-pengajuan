                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($sakit->setujui == 'tunggu' && $sakit->disetujui_oleh == Auth::user()->id)
                                            <form action="{{ route('manager-operasional.approve.sakit.disetujui-terima', $sakit->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                <button type="submit" class="btn btn-primary btn-sm">Setujui</button>
                                            </form>
                                            <form action="{{ route('manager-operasional.approve.sakit.disetujui-tolak', $sakit->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                <button type="submit" class="btn btn-danger btn-sm">Tolak</button>
                                            </form>
                                        @endif
                                        @if($sakit->ketahui == 'tunggu' && $sakit->diketahui_oleh == Auth::user()->id)
                                            <form action="{{ route('manager-operasional.approve.sakit.diketahui-terima', $sakit->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                <button type="submit" class="btn btn-primary btn-sm">Ketahui</button>
                                            </form>
                                            <form action="{{ route('manager-operasional.approve.sakit.diketahui-tolak', $sakit->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                <button type="submit" class="btn btn-danger btn-sm">Tolak</button>
                                            </form>
                                        @endif
