                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($izin->setujui == 'tunggu' && $izin->disetujui_oleh == Auth::user()->id)
                                            <form action="{{ route('area-manager.izin.disetujui.terima', $izin->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                <button type="submit" class="btn btn-primary btn-sm">Setujui</button>
                                            </form>
                                            <form action="{{ route('area-manager.izin.disetujui.tolak', $izin->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                <button type="submit" class="btn btn-danger btn-sm">Tolak</button>
                                            </form>
                                        @endif
                                        @if($izin->ketahui == 'tunggu' && $izin->diketahui_oleh == Auth::user()->id)
                                            <form action="{{ route('area-manager.izin.diketahui.terima', $izin->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                <button type="submit" class="btn btn-primary btn-sm">Ketahui</button>
                                            </form>
                                            <form action="{{ route('area-manager.izin.diketahui.tolak', $izin->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                <button type="submit" class="btn btn-danger btn-sm">Tolak</button>
                                            </form>
                                        @endif
