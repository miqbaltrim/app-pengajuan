        ...
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        @if($totalCuti > 0)
                            <a href="{{ route('area-manager.pengajuan-cuti.create') }}" class="btn btn-success">Ajukan Cuti</a>
                        @else
                            <p>Maaf, Anda tidak memiliki cuti tersisa.</p>
                            <form action="{{ route('area-manager.pengajuan-cuti.reset') }}" method="POST" style="display: inline;">
                                @csrf
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($ajucuti->status != 'ditolak' && $ajucuti->status == 'disetujui')
                                            <a href="{{ route('area-manager.pengajuan-cuti.view', $ajucuti->id) }}" class="btn btn-success btn-sm">View</a>
                                        @endif
                                        @if($ajucuti->status != 'ditolak' && $ajucuti->status != 'disetujui')
                                            <a href="{{ route('area-manager.pengajuan-cuti.edit', $ajucuti->id) }}" class="btn btn-primary btn-sm">Edit</a>
                                            <form action="{{ route('area-manager.pengajuan-cuti.destroy', $ajucuti->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('DELETE')
